...
{
    public partial class cp_cdi1_tabbll : BLLBase<CP_CDI1_TAB, cp_cdi1_tabdal>
    {

        public IList<CP_CDI1_TAB> GetList(int cd_id)
        {
            return dal.GetList(t => t.CHILD_ID == cd_id, t => t.UPDATE_TIME, false);
        }


        public CP_CDI1_TAB GetByCdId(int cd_id)
        {
        ...
        public bool SaveOrUpdate(CP_CDI1_TAB obj)
        {
            if (obj.ID != 0)
        ...
        public CP_CDI1_TAB Get(int id, string type)
        {
            return dal.Get(t => t.ID == id && t.TYPE == type);
        }

        public CP_CDI1_TAB Get(int cd_id, string type, int externalid)
        {
