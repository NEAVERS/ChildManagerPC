...
{
    public partial class cp_dst_printer : UserControl
    {
        private Rectangle _rectBody;

        private TB_CHILDBASE _baseobj = null;
        private CP_DST_TAB _obj = null;

        public cp_dst_printer(TB_CHILDBASE baseobj, CP_DST_TAB obj)
        {
            InitializeComponent();
            _baseobj = baseobj;
            _obj = obj;
        ...
        private void PrintBody(Graphics g)
        {
            ...
            MiddleLeftPrintText("DST测试结果", new Font("宋体", 14f, FontStyle.Bold), new SolidBrush(Color.Black), rect, g);//

            top += 60;
            left = _rectBody.Left;
            rect = new Rectangle(left, top, _rectBody.Width, 30);
            MiddleLeftPrintText("原始总分："+_obj.DST_YSZF, rect, g);

            top += 60;
            left = _rectBody.Left;
            rect = new Rectangle(left, top, _rectBody.Width, 30);
            MiddleLeftPrintText("DQ：" + _obj.DST_DQ, rect, g);

            top += 60;
            left = _rectBody.Left;
            rect = new Rectangle(left, top, _rectBody.Width, 30);
            MiddleLeftPrintText("结果：" + _obj.DST_JG, rect, g);
