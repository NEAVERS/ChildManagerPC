...
{
    public partial class PanelYingyangzhuananPrinter : UserControl
    {
        ...
        public PanelYingyangzhuananPrinter(DataGridView dataGridView, ChildBaseInfoObj baseobj, ChildYingyanggeanObj yinyanggeanobj)
        {
            InitializeComponent();
        ...
        public void Print(string printer, bool isPreview)
        {
            ...
            printDoc.DocumentName = "蛋白质-能量营养不良儿童专案管理记录";
        ...
        private void CountPageNos(Graphics g)
        {
            ...
            _columnsPrintWidth.Add(50);
            _columnsPrintWidth.Add(50);
            _columnsPrintWidth.Add(200);
        ...
        private void PrintHeaders(Graphics g)
        {
            ...
            using (Brush brush = new SolidBrush(Color.Black))
            {
                ...
                if (_yinyanggeanobj != null)
                {
                    ...
                    rect = new Rectangle(left, top, 80, _patientInfoHeight);
                    g.DrawString("出生史:", new Font("Arial Unicode MS", 11f), brush, rect, sfleft);
                    left += 80;
                    ...
                    g.DrawString("6个月内喂养史:", new Font("Arial Unicode MS", 11f), brush, rect, sfleft);
                    left += 130;

                    rect = new Rectangle(left, top, 165, _patientInfoHeight);
                    g.DrawString(_yinyanggeanobj.weiyangshi, new Font("Arial Unicode MS", 11f), brush, rect, sf);
                    g.DrawLine(pen, rect.Left, rect.Bottom - lineOffset, rect.Right, rect.Bottom - lineOffset);
                    left += 165;

                    left += 225;
                    rect = new Rectangle(left, top, 150, _patientInfoHeight);
                    g.DrawString("开始食物转换年龄:", new Font("Arial Unicode MS", 11f), brush, rect, sf);
                    left += 150;
        ...
        private void PrintColumnsHeader(Graphics g)
        {
            ...
            using (StringFormat sf = new StringFormat())
            {
                ...
                for (int i = 0; i < _dataGridView.Columns.Count; i++)
                {
                    ...
                    if (i == 2)
                    {
                        Rectangle rectColumnHeaderCell = new Rectangle(x, _rectBody.Top, 120, 40);
                        g.DrawRectangle(pen, rectColumnHeaderCell);

                        Rectangle rectColumnHeaderCellx1 = new Rectangle(x, _rectBody.Top + 5, 120, 40);

                        g.DrawString("体格检查", font, brush, rectColumnHeaderCellx1, sf);

                        Rectangle rectColumnHeaderCell1 = new Rectangle(x, _rectBody.Top + 40, _columnsPrintWidth[i], 40);
                        g.DrawRectangle(pen, rectColumnHeaderCell1);
                        g.DrawString(_dataGridView.Columns[i].HeaderText, font, brush, rectColumnHeaderCell1, sf);
                    }
                    else if (i == 3)
                    {
                        Rectangle rectColumnHeaderCell = new Rectangle(x, _rectBody.Top + 40, _columnsPrintWidth[i], 40);
                        g.DrawRectangle(pen, rectColumnHeaderCell);
