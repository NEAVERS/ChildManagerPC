...
{
    public partial class cp_dst_panel : UserControl
    {
        private cp_dst_tabbll bll = new cp_dst_tabbll();//儿童建档基本信息业务处理类
        tab_person_databll personbll = new tab_person_databll();
        cp_WomenInfo _cpwomeninfo = null;
        private bool _isShowTopPanel;
        IList<CP_DST_TAB> _list = null;
        private CP_DST_TAB _obj;
        ...
        public cp_dst_panel(cp_WomenInfo cpwomeninfo)
        {
            InitializeComponent();
        ...
        public cp_dst_panel(cp_WomenInfo cpwomeninfo, bool isShowTopPanel) : this(cpwomeninfo)
        {
        ...
        private CP_DST_TAB getObj()
        {
            if (cszqm.Text.Trim() == "")
            {
                cszqm.Text = globalInfoClass.UserName;
            }
            CP_DST_TAB obj = CommonHelper.GetObjMenzhen<CP_DST_TAB>(panel1.Controls, _cpwomeninfo.cd_id);
        ...
        {
            CommonHelper.setForm(new CP_DST_TAB(), panel1.Controls);
        }
        ...
        {
            Cursor.Current = Cursors.WaitCursor;
            CP_DST_TAB obj = bll.Get(_obj?.ID ?? 0);
            if (obj == null)
            {
            ...
            try
            {
                ...
                cp_dst_printer printer = new cp_dst_printer(baseobj, obj);
                printer.Print(ispre);
            }
            catch (Exception ex)
            {
