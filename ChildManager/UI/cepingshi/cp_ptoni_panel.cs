...
{
    public partial class cp_ptoni_panel : UserControl
    {
        cp_ptoni_tabbll bll = new cp_ptoni_tabbll();//儿童建档基本信息业务处理类
        tab_person_databll personbll = new tab_person_databll();
        cp_WomenInfo _cpwomeninfo = null;
        private bool _isShowTopPanel;
        IList<CP_PTONI_TAB> _list = null;
        private CP_PTONI_TAB _obj;
        ...
        public cp_ptoni_panel(cp_WomenInfo cpwomeninfo)
        {
            InitializeComponent();
        ...
        public cp_ptoni_panel(cp_WomenInfo cpwomeninfo, bool isShowTopPanel) : this(cpwomeninfo)
        {
        ...
        private CP_PTONI_TAB getObj()
        {
            if (cszqm.Text.Trim() == "")
            {
                cszqm.Text = globalInfoClass.UserName;
            }
            CP_PTONI_TAB obj = CommonHelper.GetObjMenzhen<CP_PTONI_TAB>(panel1.Controls, _cpwomeninfo.cd_id);
        ...
        {
            CommonHelper.setForm(new CP_PTONI_TAB(), panel1.Controls);
        }
        ...
        {
            Cursor.Current = Cursors.WaitCursor;
            CP_PTONI_TAB obj = bll.Get(_obj?.ID ?? 0);
            if (obj == null)
            {
            ...
            try
            {
                ...
                cp_ptoni_printer printer = new cp_ptoni_printer(baseobj, obj);
