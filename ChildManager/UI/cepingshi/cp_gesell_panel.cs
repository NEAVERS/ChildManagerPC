...
{
    public partial class cp_gesell_panel : UserControl
    {
        private cp_gesell_tabbll bll = new cp_gesell_tabbll();//儿童建档基本信息业务处理类
        tab_person_databll personbll = new tab_person_databll();
        cp_WomenInfo _cpwomeninfo = null;
        private bool _isShowTopPanel;
        IList<CP_GESELL_TAB> _list = null;
        private CP_GESELL_TAB _obj;
        ...
        public cp_gesell_panel(cp_WomenInfo cpwomeninfo)
        {
            InitializeComponent();
        ...
        public cp_gesell_panel(cp_WomenInfo cpwomeninfo, bool isShowTopPanel) : this(cpwomeninfo)
        {
        ...
        private CP_GESELL_TAB getObj()
        {
            if (cszqm.Text.Trim() == "")
            {
                cszqm.Text = globalInfoClass.UserName;
            }
            CP_GESELL_TAB obj = CommonHelper.GetObjMenzhen<CP_GESELL_TAB>(panel1.Controls, _cpwomeninfo.cd_id);
        ...
        {
            CommonHelper.setForm(new CP_GESELL_TAB(), panel1.Controls);
        ...
        {
            Cursor.Current = Cursors.WaitCursor;
            CP_GESELL_TAB obj = bll.Get(_obj?.ID ?? 0);
            if (obj == null)
            {
            ...
            try
            {
                ...
                cp_gesell_printer printer = new cp_gesell_printer(baseobj, obj);
                printer.Print(ispre);
            }
            catch (Exception ex)
            {
