...
{
    public partial class cp_sm_panel : UserControl
    {
        private cp_sm_tabbll bll = new cp_sm_tabbll();//儿童建档基本信息业务处理类
        tab_person_databll personbll = new tab_person_databll();
        cp_WomenInfo _cpwomeninfo = null;
        private bool _isShowTopPanel;
        IList<CP_SM_TAB> _list = null;
        private CP_SM_TAB _obj;
        ...
        public cp_sm_panel(cp_WomenInfo cpwomeninfo)
        {
            InitializeComponent();
        ...
        public cp_sm_panel(cp_WomenInfo cpwomeninfo, bool isShowTopPanel) : this(cpwomeninfo)
        {
        ...
        private CP_SM_TAB getObj()
        {
            if (cszqm.Text.Trim() == "")
            {
                cszqm.Text = globalInfoClass.UserName;
            }
            CP_SM_TAB obj = CommonHelper.GetObjMenzhen<CP_SM_TAB>(panel1.Controls, _cpwomeninfo.cd_id);
        ...
        {
            CommonHelper.setForm(new CP_SM_TAB(), panel1.Controls);
        ...
        {
            Cursor.Current = Cursors.WaitCursor;
            CP_SM_TAB obj = bll.Get(_obj?.ID ?? 0);
            if (obj == null)
            {
            ...
            try
            {
                ...
                cp_sm_printer printer = new cp_sm_printer(baseobj, obj);
                printer.Print(ispre);
            }
            catch (Exception ex)
            {
		...
		void BzfTextChanged(object sender, EventArgs e)
		{
			int bzfindex = 0;
			int.TryParse(bzf.Text.Trim(),out bzfindex);
			checkBox4.Checked=bzfindex==10;
			checkBox3.Checked=bzfindex==9;
			checkBox1.Checked=bzfindex==8;
			checkBox2.Checked=bzfindex==7;
			checkBox5.Checked=bzfindex==6;
			checkBox6.Checked=bzfindex==5;
		}
