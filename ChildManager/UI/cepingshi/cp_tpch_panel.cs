...
{
    public partial class cp_tpch_panel : UserControl
    {
        private cp_tpch_tabbll bll = new cp_tpch_tabbll();//儿童建档基本信息业务处理类
        tab_person_databll personbll = new tab_person_databll();
        cp_WomenInfo _cpwomeninfo = null;
        private bool _isShowTopPanel;
        IList<CP_TPCH_TAB> _list = null;
        private CP_TPCH_TAB _obj;
        ...
        public cp_tpch_panel(cp_WomenInfo cpwomeninfo)
        {
            InitializeComponent();
        ...
        public cp_tpch_panel(cp_WomenInfo cpwomeninfo, bool isShowTopPanel) : this(cpwomeninfo)
        {
        ...
        private CP_TPCH_TAB getObj()
        {
            if (cszqm.Text.Trim() == "")
            {
                cszqm.Text = globalInfoClass.UserName;
            }
            CP_TPCH_TAB obj = CommonHelper.GetObjMenzhen<CP_TPCH_TAB>(panel1.Controls, _cpwomeninfo.cd_id);
        ...
        {
            CommonHelper.setForm(new CP_TPCH_TAB(), panel1.Controls);
        }
        ...
        {
            Cursor.Current = Cursors.WaitCursor;
            CP_TPCH_TAB obj = bll.Get(_obj?.ID ?? 0);
            if (obj == null)
            {
            ...
            try
            {
                ...
                cp_tpch_printer printer = new cp_tpch_printer(baseobj, obj);
                printer.Print(ispre);
            }
            catch (Exception ex)
            {
