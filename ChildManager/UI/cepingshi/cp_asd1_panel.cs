...
{
    public partial class cp_asd1_panel : UserControl
    {
        private cp_asd1_tabbll bll = new cp_asd1_tabbll();//儿童建档基本信息业务处理类
        tab_person_databll personbll = new tab_person_databll();
        cp_WomenInfo _cpwomeninfo = null;
        private bool _isShowTopPanel;
        IList<CP_ASD1_TAB> _list = null;
        private CP_ASD1_TAB _obj;
        ...
        public cp_asd1_panel(cp_WomenInfo cpwomeninfo)
        {
            InitializeComponent();
        ...
        public cp_asd1_panel(cp_WomenInfo cpwomeninfo, bool isShowTopPanel) : this(cpwomeninfo)
        {
        ...
        private CP_ASD1_TAB getObj()
        {
            if (cszqm.Text.Trim() == "")
            {
                cszqm.Text = globalInfoClass.UserName;
            }
            CP_ASD1_TAB obj = CommonHelper.GetObjMenzhen<CP_ASD1_TAB>(panel1.Controls, _cpwomeninfo.cd_id);
        ...
        {
            CommonHelper.setForm(new CP_ASD1_TAB(), panel1.Controls);
        ...
        {
            Cursor.Current = Cursors.WaitCursor;
            CP_ASD1_TAB obj = bll.Get(_obj?.ID ?? 0);
            if (obj == null)
            {
            ...
            try
            {
                ...
                cp_asd1_printer printer = new cp_asd1_printer(baseobj, obj);
                printer.Print(ispre);
            }
            catch (Exception ex)
            {
		...
		void M_chat_gwxmTextChanged(object sender, EventArgs e)
		{
			int gwxm = 0;
			int ryxm = 0;
			int.TryParse(m_chat_gwxm.Text.Trim(),out gwxm);
			int.TryParse(m_chat_ryxm.Text.Trim(),out ryxm);
			checkBox2.Checked= gwxm >= 2 || ryxm >= 3;
			checkBox1.Checked= !checkBox2.Checked;
		}
		void Abc_dfTextChanged(object sender, EventArgs e)
		{
			int df = 0;
			int.TryParse(abc_df.Text.Trim(),out df);
			checkBox4.Checked= df < 53;
			checkBox7.Checked= df >= 53 && df < 67;
			checkBox3.Checked= df >= 67;
		}
