...
{
    public partial class cp_gmyjc_panel : UserControl
    {
        private cp_gmyjc_tabbll bll = new cp_gmyjc_tabbll();//儿童建档基本信息业务处理类
        tab_person_databll personbll = new tab_person_databll();
        cp_WomenInfo _cpwomeninfo = null;
        private bool _isShowTopPanel;
        IList<CP_GMYJC_TAB> _list = null;
        private CP_GMYJC_TAB _obj;
        ...
        public cp_gmyjc_panel(cp_WomenInfo cpwomeninfo)
        {
            InitializeComponent();
            _cpwomeninfo = cpwomeninfo;
            CommonHelper.SetAllControls(panel1, true);
        ...
        public cp_gmyjc_panel(cp_WomenInfo cpwomeninfo, bool isShowTopPanel) : this(cpwomeninfo)
        {
        ...
        private CP_GMYJC_TAB getObj()
        {
            if (cszqm.Text.Trim() == "")
            {
                cszqm.Text = globalInfoClass.UserName;
            }
            CP_GMYJC_TAB obj = CommonHelper.GetObjMenzhen<CP_GMYJC_TAB>(panel1.Controls, _cpwomeninfo.cd_id);
        ...
        {
            CommonHelper.setForm(new CP_GMYJC_TAB(), panel1.Controls);
        }
        ...
        {
            Cursor.Current = Cursors.WaitCursor;
            CP_GMYJC_TAB obj = bll.Get(_obj?.ID ?? 0);
            if (obj == null)
            {
            ...
            try
            {
                ...
                cp_gmyjc_printer printer = new cp_gmyjc_printer(baseobj, obj);
                printer.Print(ispre);
            }
            catch (Exception ex)
            {
        ...
        public void SetResult(string qiuzhen, Control control)
        {
            if (!string.IsNullOrEmpty(qiuzhen))
            {
                ...
                if (qiuzhen.Contains('/'))
                {
                    ...
                    int qzNum1 = Convert.ToInt32(strQiuzhen[0]);
                    int qzNum2 = Convert.ToInt32(strQiuzhen[1]);
                    if (qzNum1 == 0 && qzNum2 == 0)
                    {
                        control.Text = "";
                    }
                    else if ((qzNum1 > 0 && qzNum1 <= 2) || (qzNum1 == 0 && qzNum2 > 0))
                    {
                        control.Text = "±";
                    }
                    else if (qzNum1 >= 3 && qzNum1 <= 5)
                    {
                        control.Text = "+";
                    }
                    else if (qzNum1 >= 6 && qzNum1 <= 8)
                    {
                        control.Text = "++";
                    }
                    else if (qzNum1 > 8)
