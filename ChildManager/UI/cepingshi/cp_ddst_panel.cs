...
{
    public partial class cp_ddst_panel : UserControl
    {
        private cp_ddst_tabbll bll = new cp_ddst_tabbll();//儿童建档基本信息业务处理类
        tab_person_databll personbll = new tab_person_databll();
        cp_WomenInfo _cpwomeninfo = null;
        private bool _isShowTopPanel;
        IList<CP_DDST_TAB> _list = null;
        private CP_DDST_TAB _obj;
        private string _type = "DDST";
        ...
        public cp_ddst_panel(cp_WomenInfo cpwomeninfo)
        {
            InitializeComponent();
        ...
        #endregion
        public cp_ddst_panel(cp_WomenInfo cpwomeninfo, bool isShowTopPanel) : this(cpwomeninfo)
        {
        ...
        private CP_DDST_TAB getObj()
        {
            if (cszqm.Text.Trim() == "")
            {
                cszqm.Text = globalInfoClass.UserName;
            }
            CP_DDST_TAB obj = CommonHelper.GetObjMenzhen<CP_DDST_TAB>(panel1.Controls, _cpwomeninfo.cd_id);
            obj.TYPE = _type;
        ...
        {
            Cursor.Current = Cursors.WaitCursor;
            CP_DDST_TAB obj = bll.Get(_obj?.ID ?? 0);
            if (obj == null)
            {
            ...
            try
            {
                ...
                cp_ddst_printer printer = new cp_ddst_printer(baseobj, obj);
                printer.Print(ispre);
            }
            catch (Exception ex)
            {
