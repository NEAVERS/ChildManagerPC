...
{
    public partial class cp_asd2_panel : UserControl
    {
        private cp_asd2_tabbll bll = new cp_asd2_tabbll();//儿童建档基本信息业务处理类
        tab_person_databll personbll = new tab_person_databll();
        cp_WomenInfo _cpwomeninfo = null;
        private bool _isShowTopPanel;
        IList<CP_ASD2_TAB> _list = null;
        private CP_ASD2_TAB _obj;
        ...
        public cp_asd2_panel(cp_WomenInfo cpwomeninfo)
        {
            InitializeComponent();
        ...
        public cp_asd2_panel(cp_WomenInfo cpwomeninfo, bool isShowTopPanel) : this(cpwomeninfo)
        {
        ...
        private CP_ASD2_TAB getObj()
        {
            if (cszqm.Text.Trim() == "")
            {
                cszqm.Text = globalInfoClass.UserName;
            }
            CP_ASD2_TAB obj = CommonHelper.GetObjMenzhen<CP_ASD2_TAB>(panel1.Controls, _cpwomeninfo.cd_id);
        ...
        {
            CommonHelper.setForm(new CP_ASD2_TAB(), panel1.Controls);
        ...
        {
            Cursor.Current = Cursors.WaitCursor;
            CP_ASD2_TAB obj = bll.Get(_obj?.ID ?? 0);
            if (obj == null)
            {
            ...
            try
            {
                ...
                cp_asd2_printer printer = new cp_asd2_printer(baseobj, obj);
                printer.Print(ispre);
            }
            catch (Exception ex)
            {
        ...
        void Srs_zjTextChanged(object sender, EventArgs e)
        {

            int zlb = 0;
            int.TryParse(srs_zlb.Text.Trim(), out zlb);
            checkBox1.Checked = zlb < 65;
