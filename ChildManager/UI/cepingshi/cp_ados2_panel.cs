...
{
    public partial class cp_ados2_panel : UserControl
    {
        private cp_ados2_tabbll bll = new cp_ados2_tabbll();//儿童建档基本信息业务处理类
        tab_person_databll personbll = new tab_person_databll();
        cp_WomenInfo _cpwomeninfo = null;
        private bool _isShowTopPanel;
        IList<cp_ados2_tab> _list = null;
        private cp_ados2_tab _obj;
        ...
        public cp_ados2_panel(cp_WomenInfo cpwomeninfo)
        {
            InitializeComponent();
        ...
        public cp_ados2_panel(cp_WomenInfo cpwomeninfo, bool isShowTopPanel) : this(cpwomeninfo)
        {
        ...
        private cp_ados2_tab getObj()
        {
            if (pgzqm.Text.Trim() == "")
            {
                pgzqm.Text = globalInfoClass.UserName;
            }
            cp_ados2_tab obj = CommonHelper.GetObjMenzhen<cp_ados2_tab>(panel1.Controls, _cpwomeninfo.cd_id);
        ...
        {
            CommonHelper.setForm(new cp_ados2_tab(), panel1.Controls);
        ...
        {
            Cursor.Current = Cursors.WaitCursor;
            cp_ados2_tab obj = bll.Get(_obj?.id ?? 0);
            if (obj == null)
            {
            ...
            try
            {
                ...
                cp_ados2_printer printer = new cp_ados2_printer(baseobj, obj);
                printer.Print(ispre);
            }
            catch (Exception ex)
            {
        ...
        private void gt_count(object sender, EventArgs e)
        {
            ...
            int.TryParse(gt_zdbd.Text.Trim(), out i_gt_gt_zdbd);
            int.TryParse(gt_zwdz.Text.Trim(), out i_gt_zwdz);
            int gtcou = i_gt_hh + i_gt_kb + i_gt_msxdz + i_gt_gt_zdbd + i_gt_zwdz;
            ...
            int.TryParse(hd_xhxshgt.Text.Trim(), out i_hd_xhxshgt);
            int hdcou = i_hd_bxc + i_hd_cxtr + i_hd_gx + i_hd_sjfy + i_hd_zdbd + i_hd_zfdzd + i_hd_xhxshgt;
