...
{
    public partial class cp_wisc_panel : UserControl
    {
        private cp_wisc_tabbll bll = new cp_wisc_tabbll();//儿童建档基本信息业务处理类
        tab_person_databll personbll = new tab_person_databll();
        cp_WomenInfo _cpwomeninfo = null;
        private bool _isShowTopPanel;
        IList<CP_WISC_TAB> _list = null;
        private CP_WISC_TAB _obj;
        ...
        public cp_wisc_panel(cp_WomenInfo cpwomeninfo)
        {
            InitializeComponent();
        ...
        public cp_wisc_panel(cp_WomenInfo cpwomeninfo, bool isShowTopPanel) : this(cpwomeninfo)
        {
        ...
        private CP_WISC_TAB getObj()
        {
            if (cszqm.Text.Trim() == "")
            {
                cszqm.Text = globalInfoClass.UserName;
            }
            CP_WISC_TAB obj = CommonHelper.GetObjMenzhen<CP_WISC_TAB>(panel1.Controls, _cpwomeninfo.cd_id);
        ...
        {
            CommonHelper.setForm(new CP_WISC_TAB(), panel1.Controls);
        ...
        {
            Cursor.Current = Cursors.WaitCursor;
            CP_WISC_TAB obj = bll.Get(_obj?.ID ?? 0);
            if (obj == null)
            {
            ...
            try
            {
                ...
                cp_wisc_printer printer = new cp_wisc_printer(baseobj, obj);
                printer.Print(ispre);
            }
            catch (Exception ex)
            {
		...
		void Yycy_zsTextChanged(object sender, EventArgs e)
		{
			...
			int.TryParse(yycy_fl.Text.Trim(),out fl);
			int.TryParse(yycy_ss.Text.Trim(),out ss);
			int.TryParse(yycy_ch.Text.Trim(),out ch);
			int.TryParse(yycy_lw.Text.Trim(),out lw);
			int.TryParse(yycy_szgd.Text.Trim(),out szgd);
			yycy_sum.Text= (zs+fl+ss+ch+lw+szgd).ToString();
		}
		void Czcy_ttTextChanged(object sender, EventArgs e)
		{
			int tt = 0;
			int tppl = 0;
			int mkt = 0;
			int txpc = 0;
			int bm = 0;
			int.TryParse(czcy_tt.Text.Trim(),out tt);
			int.TryParse(czcy_tppl.Text.Trim(),out tppl);
			int.TryParse(czcy_mkt.Text.Trim(),out mkt);
			int.TryParse(czcy_txpc.Text.Trim(),out txpc);
			int.TryParse(czcy_bm.Text.Trim(),out bm);
			czcy_sum.Text = (tt + tppl + mkt + txpc + bm).ToString();
		}
