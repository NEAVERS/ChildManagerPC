...
{
    public partial class cp_ados4_panel : UserControl
    {
        private cp_ados4_tabbll bll = new cp_ados4_tabbll();//儿童建档基本信息业务处理类
        tab_person_databll personbll = new tab_person_databll();
        cp_WomenInfo _cpwomeninfo = null;
        private bool _isShowTopPanel;
        IList<CP_ADOS4_TAB> _list = null;
        private CP_ADOS4_TAB _obj;
        ...
        public cp_ados4_panel(cp_WomenInfo cpwomeninfo)
        {
            InitializeComponent();
        ...
        public cp_ados4_panel(cp_WomenInfo cpwomeninfo, bool isShowTopPanel) : this(cpwomeninfo)
        {
        ...
        private CP_ADOS4_TAB getObj()
        {
            if (pgzqm.Text.Trim() == "")
            {
                pgzqm.Text = globalInfoClass.UserName;
            }
            CP_ADOS4_TAB obj = CommonHelper.GetObjMenzhen<CP_ADOS4_TAB>(panel1.Controls, _cpwomeninfo.cd_id);
        ...
        {
            CommonHelper.setForm(new CP_ADOS4_TAB(), panel1.Controls);
        ...
        {
            Cursor.Current = Cursors.WaitCursor;
            CP_ADOS4_TAB obj = bll.Get(_obj?.ID ?? 0);
            if (obj == null)
            {
            ...
            try
            {
                ...
                cp_ados4_printer printer = new cp_ados4_printer(baseobj, obj);
                printer.Print(ispre);
            }
            catch (Exception ex)
            {
        ...
        private void gt_count(object sender, EventArgs e)
        {
            ...
            int.TryParse(gt_jjxdz.Text.Trim(), out i_gt_jjxdz);
            int gtcou = i_gt_hh + i_gt_kb + i_gt_msxdz + i_gt_jjxdz;
            ...
            int.TryParse(hd_dtrqx.Text.Trim(), out i_hd_dtrqx);
            int.TryParse(hd_sjfy.Text.Trim(), out i_hd_sjfy);
            int.TryParse(hd_zdbd.Text.Trim(), out i_hd_zdbd);
            int.TryParse(hd_zr.Text.Trim(), out i_hd_zr);
            int.TryParse(hd_xhxshgt.Text.Trim(), out i_hd_xhxshgt);
            int hdcou = i_hd_bxc + i_hd_cxtr + i_hd_dtrqx + i_hd_sjfy + i_hd_zdbd + i_hd_zr + i_hd_xhxshgt;
